...
$$y= \alpha + f(x)\beta + \epsilon$$
--

<br>
Or more formally _(Wood, 2017)_:

$$ g(\mu i) = Ai\theta + f1(x1) + f2(x2i) + f3(x3i, x4i) + ...$$
__Where:__
+ $\mu i \equiv E(Y i)$, _the expectation of Y_
+ $Yi \sim EF(\mu i, \phi i)$, $Yi$ _is a response variable, distributed according to exponential family distribution with mean_ $\mu i$ _and shape parameter_ $\phi$.
+ $Ai$ _is a row of the model matrix for any strictly parametric model components with_ $\theta$ _the corresponding parameter vector._
+ $fi$ _are smooth functions of the covariates,_ $xk$, _where_ $k$ _is each function basis._
...
+ _Hastie (1985)_ method uses knot every point, _Wood (2017)_ uses reduced-rank version
